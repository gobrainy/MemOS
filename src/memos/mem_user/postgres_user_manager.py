...
class User(Base):
    ...
    # Store enum as string for portability across dialects
...
class PostgresUserManager:
    ...
    def __init__(
        ...
        """Initialize the user manager with PostgreSQL database connection.

        Args:
            user_id (str, optional): User ID. If None, uses default user ID.
            host (str): PostgreSQL server host. Defaults to "localhost".
            port (int): PostgreSQL server port. Defaults to 5432.
            username (str): PostgreSQL username. Defaults to "postgres".
            password (str): PostgreSQL password. Defaults to "".
            database (str): PostgreSQL database name. Defaults to "memos_users".
            sslmode (str | None): Optional sslmode parameter.
        """
        # Build PostgreSQL connection URL
        auth_part = f"{username}:{password}@" if password else f"{username}@"
        base_url = f"postgresql+psycopg://{auth_part}{host}:{port}/{database}"
        if sslmode:
            base_url = f"{base_url}?sslmode={sslmode}"

        self.connection_url = base_url
        ...
        logger.info(
            f"PostgresUserManager initialized with database at {host}:{port}/{database}"
        )
    ...
    def _init_root_user(self, user_id: str) -> None:
        ...
        try:
            ...
            if user_count == 0:
                root_user = User(user_id=user_id, user_name=user_id, role=UserRole.ROOT.value)
    ...
    def close(self) -> None:
        ...
        This method should be called when the PostgresUserManager is no longer needed
